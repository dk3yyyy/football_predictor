...
def predict_real_match(home_query: str, away_query: str):
    ...
    home_matches = df[(df['home_team_name'] == home_name) | (df['away_team_name'] == home_name)].sort_values('utc_date')
    ...
    # Same for Away team
    away_matches = df[(df['home_team_name'] == away_name) | (df['away_team_name'] == away_name)].sort_values('utc_date')
    ...
    print("="*50)
    print(f"📈 Form [{home_name}]: {h_pts:.1f} pts in last 5 ({h_gf:.1f} scored, {h_ga:.1f} conceded)")
    print(f"📈 Form [{away_name}]: {a_pts:.1f} pts in last 5 ({a_gf:.1f} scored, {a_ga:.1f} conceded)")
    print("-"*50)

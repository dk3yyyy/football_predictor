...
class FeaturePipeline:
    ...
    def compute_rolling_stats(self, df: pd.DataFrame, window: int = 5) -> pd.DataFrame:
        ...
        home_logs = df[['match_id', 'utc_date', 'home_team_name', 'home_goals_ft', 'away_goals_ft', 'home_points']].copy()
        home_logs.columns = ['match_id', 'date', 'team_name', 'goals_scored', 'goals_conceded', 'points']
        home_logs['is_home'] = 1

        away_logs = df[['match_id', 'utc_date', 'away_team_name', 'away_goals_ft', 'home_goals_ft', 'away_points']].copy()
        away_logs.columns = ['match_id', 'date', 'team_name', 'goals_scored', 'goals_conceded', 'points']
        away_logs['is_home'] = 0
        ...
        team_logs['rolling_points'] = grouped['points'].transform(lambda x: x.rolling(window).sum().shift(1))
        team_logs['rolling_gf'] = grouped['goals_scored'].transform(lambda x: x.rolling(window).sum().shift(1))
        team_logs['rolling_ga'] = grouped['goals_conceded'].transform(lambda x: x.rolling(window).sum().shift(1))
        
        # Calculate Fatigue (rest days)
        team_logs['days_rest'] = grouped['date'].diff().dt.days.shift(1).fillna(10) # default 10 days for start of season

        # Format back to attach to matches
        home_features = team_logs[team_logs['is_home'] == 1][['match_id', 'rolling_points', 'rolling_gf', 'rolling_ga', 'days_rest']]
        home_features.columns = ['match_id', 'home_rolling_points', 'home_rolling_gf', 'home_rolling_ga', 'home_days_rest']

        away_features = team_logs[team_logs['is_home'] == 0][['match_id', 'rolling_points', 'rolling_gf', 'rolling_ga', 'days_rest']]
        away_features.columns = ['match_id', 'away_rolling_points', 'away_rolling_gf', 'away_rolling_ga', 'away_days_rest']
    ...
    def attach_xg_stats(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        with self.db.engine.connect() as conn:
            xg_df = pd.read_sql(text("SELECT team_name, xg, xga, xg_diff, scraped_at FROM xg_stats"), conn)
        ...
        df = df.rename(columns={'xg': 'home_xg', 'xga': 'home_xga', 'xg_diff': 'home_xg_diff'}).drop(columns=['team_name'], errors='ignore')
        
        df = df.merge(latest_xg, left_on='away_team_name', right_on='team_name', how='left')
        df = df.rename(columns={'xg': 'away_xg', 'xga': 'away_xga', 'xg_diff': 'away_xg_diff'}).drop(columns=['team_name'], errors='ignore')
    ...
    def build_fixtures_predict_set(self) -> pd.DataFrame:
        ...
        """
        with self.db.engine.connect() as conn:
            df = pd.read_sql(text("SELECT * FROM matches WHERE status IN ('SCHEDULED', 'TIMED') ORDER BY utc_date ASC"), conn)

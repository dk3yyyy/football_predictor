...
def get_team_features(pipeline, team_name):
    ...
    with pipeline.db.engine.connect() as conn:
        ...
        xg_rec = conn.execute(
            text(
                "SELECT xg, xga FROM xg_stats WHERE team_name = :t ORDER BY scraped_at DESC LIMIT 1"
            ),
            {"t": db_name},
        ).fetchone()
    ...
    days_rest = (
        pd.Timestamp.now(tz="UTC").replace(tzinfo=None) - last_date.replace(tzinfo=None)
    ).days
...
def predict_custom_batch(matches_list):
    ...
    for r in results:
        ...
        print(
            f"Win%: {r['home']} {r['prob_home'] * 100:.1f}% | Draw {r['prob_draw'] * 100:.1f}% | {r['away']} {r['prob_away'] * 100:.1f}%"
        )
    print("=" * 60 + "\n")

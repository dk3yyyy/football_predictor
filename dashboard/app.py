...
st.sidebar.markdown(
    "Use this panel to filter the models predictions by upcoming match dates or historical accuracy."
)
...
with tab1:
    ...
    if df_upcoming.empty:
        st.info(
            f"No predictions found for {date_filter}. Either wait for the scheduler to trigger or train the model if it's the first run."
        )
    else:
...
with tab2:
    ...
    if not df_value.empty:
        for _idx, row in df_value.iterrows():
